...
<</Count 266/Kids[3 0 R。。。298 0 R 299 0 R]/Type/Pages>>
    ...
    def decode( self, buf, cmap, desfile ) :
        try :
            ...
            retbuf = ""
            cur_xy, cellMap, textMap, tm = {}, {}, {}, {}  # 坐标信息, 表格信息, 文本信息, 转换坐标
            print(desfile)
    ...
    def buildPageTxt( self, textMap, cellMap, cellIndexMap, tableMap, colIndexMap, rowIndexMap ):
        ...
        try:
            ...
            while( not i in textMap.keys() and textMap[i]['pre'] == -1 ):
                i += 1
...
└──────────┴────┴────┴────┴────┴────┘
    ...
    # 15.12 buildRowBody()
    #      将表格中间的分割线.
    #      要根据本行与上一行的列数是否相同来调整. 如果当前列末位置正好与上一行的某个列末位置相同, 则要用"mid_mid", 如果不相同, 就用"mid_top"
    #      最后一个位置, 如果行长相同, 用"mid_right", 否则用 "right_top"
    #      要求必须不能是第一行表格, 否则会出现错误. 这儿没有容错, 如果要容错, 需要增加参数 tableMap, 访问 tableMap[TableID][RowID][0]
    #
    # 15.14 getRowSplitPos()
    #      获取指定行的分割线位置
     ...
     */
    '''
    def processBDC( buf, streambuf, cmap, cur_xy, textMap, cellMap, tm ) :
        try:
            retbuf = ""
            tmpbuf = buf
            while ( True ) :       
...
<0E2A>Tj
     ...
     */
    '''
    def processRE( self, buf, cellMap ) :
        try :
    ...
    def getOriginXY( self, cur_xy, tm ) :
        try:
            ...
            if ( len(tm) == 0 ) :      # 如果没有tm, 也就是没有转换坐标, 则x,y 就是绝对坐标
                ...
                cur_xy["ox"] = round(tm['a']*cur_xy['x'] + tm['c'] * cur_xy['y'] + tm['e'],3)
                cur_xy["oy"] = round(tm['b']*cur_xy['x'] + tm['d'] * cur_xy['y'] + tm['f'],3)
        except:
