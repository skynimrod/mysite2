...
<0E2A>Tj
    ...
    # 2017.02.09:
    #     把表格内容存放在哈希表 tableContentMap = {表编号:"表的内容(文字及分隔符)",} 表编号从1开始
        ...
        try:
            ...
            for k,v in sorted( textMap.items(), key = lambda d:d[1]['o_xy']['oy'], reverse=True ):  # 按oy 坐标, 从上往下处理
                ...
                if ( cellId == -1 ):   # 不在cell中的文本本身就是单独一行, 因为之前已经处理过文本了
                    ...
                    retbuf += tblcontent
                    # 2017.02.09 : 把表格存放到哈希表 tableContentMap中, 下标从1开始, 该表存放的处理过的所有页中的表格
                    l = len( self.tableContentMap )
                    self.tableContentMap[l+1] = tblcontent
