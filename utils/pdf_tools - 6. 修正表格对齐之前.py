        ...
        try:
            ...
            while ( True ):
                ...
                if ( objSum < 1 ):       # 这种情况几乎不可能， 这儿是为了容错
                    ...
                    if ( trailer["retcode"] < 0 or "Prev" not in buf ):  # 无效的xref之后的trailer无效或没有包含有效的xref 地址, 则出错返回
                        return xref
                    if ( trailer["Prev"] ) : 
                        print( "嵌套调用前: Trailer：" )
                        print( trailer )
...
<</Count 266/Kids[3 0 R。。。298 0 R 299 0 R]/Type/Pages>>
    ...
    def getCMAPs( self, cmaplist, xref, cmaps ):
        try:
            ...
            for item in cmaplist["total"] :           # 每一项是一个cmap 对象编号字符串
    ...
    def decode( self, buf, cmaps, desfile ) :
        try :
            ...
            retbuf = ""
            cur_xy, cellMap, cellIndexMap, textMap, tm, tf = {}, {-1:{}}, {}, {}, {}, {}  # 坐标信息, 表格信息(初始化页眉是没有的, -1用于页眉编号), 文本信息, 转换坐标
...
<0E2A>Tj
    ...
    def buildTableTxt( self, textMap, table, cellMap, cellIndexMap, colIndexMap, rowIndexMap ):
        ...
        try:
            ...
            # 表格最下面的线
            retbuf += self.buildRowFooter( rowIndexMap, colIndexMap, rows[-1], cellMap, cellIndexMap )
        except:
...
└──────────┴────┴────┴────┴────┴────┘
        ...
        try:
            ...
            for i in range(0, len_all ):
                ...
                if (  curPos in preRowSplitPos and curPos in curRowSplitPos ): # 两行都有
                    # 下面计算应该删除的多计算的分隔符的数量  getDelSplitCount( preRowSplitPos, curRowSplitPos, allSplitPos , i )
                    j = self.getDelSplitCount( preRowSplitPos, curRowSplitPos, allSplitPos , i )
                    ...
                    if ( curPos in preRowSplitPos ) :    # 在上一行
                        j = self.getDelSplitCount( preRowSplitPos, curRowSplitPos, allSplitPos , i )
                        retbuf += (con_len-j) * tableDict['rowline']    # 属于上一行的分隔符, 减去之前的当前行的分隔符数量
    ...
    # 计算表格分隔线需要删除的多计算的分隔符数量
    # 这儿需要注意,  preRowSplitPos=[0 22 42], cur..=[0 22 37 52 67]  all = [0, 22, 37, 42, 52, 67]
    # 那么对pre的第二个cell而言, 连接线及分隔符长度应该是: 42-22+1=21, 但是用all 计算的时候: 37-22+1+(42-37+1)=16+6=22， 也就是37所占的分隔符计算了2次
    #  应该减去37对应的分隔符一次. 也就是说删除的是相邻两个cell之间多余计算的分隔符部分
    # 只修正一行即可, 只要一行修正了, 另一行也就相当于修正了. 修正短的一行.
    def getDelSplitCount( self, preRowSplitPos, curRowSplitPos, allSplitPos , index ):
        count = 0
        try:
            pos= allSplitPos[index]
            short, long = [],[]

            # 1. 找短的一行, allSplitPos的最后一个元素在哪一个列表中, 那个就是长的一行
            if ( allSplitPos[-1] in curRowSplitPos  ):
                long, short = curRowSplitPos, preRowSplitPos
            else:
                long, short = preRowSplitPos, curRowSplitPos

            # 2. 下面只需要修正短的一行即可
            if ( pos in short ):  # 如果目前处理的位置是当前行的
                # 获取当前行 相邻的2个cell的位置
                curPosIndex = short.index( pos )       # 当前位置在当前行的位置列表中的索引
                curPrePos = short[curPosIndex-1]       # 当前行位置列表的前一个位置

                # 获取当前行上一个cell的分隔符在allSplitPos位置
                curPrePosIndex = allSplitPos.index( curPrePos )

                # 则 index - curPrePosIndex - 1  就是当前行之间可能存在的上一行的分隔符数量, 也就是要删除的多计算的分隔符数量
                count = index - curPrePosIndex - 1 # 多减1是因为分隔符本身是要算一次的, * 2 是因为分隔符占2个字符宽度
        except:
            print("getDelSplitCount() Exception Error!")
            traceback.print_exc()
        return count
