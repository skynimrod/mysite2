...
<0E2A>Tj
    ...
    def preProcTxtMap2( self, textMap, cellIndexMap ):
        try:
    ...
    #       这样就补齐了.
    # 2017.01.11:
    #       rowIndexMap 修改格式: {rowid:{'cells':[],'parent':prowid, 'sons':[]}}
    def preProcColRow2( self, cellMap, cellIndexMap, colMap, colIndexMap, rowMap, rowIndexMap ):
        try:
            for k,v in sorted( cellMap.items(), key=lambda d:d[1]['y'], reverse=True) :   # 按Cell的y 坐标, 从上往下处理
                ...
                if ( not row  in rowMap.values() ):  # 如果该cell的x,w没有在rowMap 中, 表示是新row
                    ...
                    rowIndexMap[key] = {'cells':[ maxlines, k],'parent':-1,'sons':[]}  # rowIndexMap 的第一个元素是当前行的最大文本行数
        ...
        try:
            for colId, v in sorted( colMap.items(), key=lambda d:d[1]['w']):    # 按列的 实际宽度来排序, 从最窄到最宽
                ...
                for rk,rv in sorted( rowIndexMap.items(), key=lambda d:d[0] ):
                    cellList = rv[1:len(rv)]                   # 跳过第一个,第一个元素是该row的最大文字行数, 后面才是cell列表
    ...
    #          
    def fixRows( self, cellIndexMap, rowMap, colIndexMap, rowIndexMap ):
            ...
            for k,v in sorted( rowMap.items(), key=lambda d:d[1]['h']):   # 按row 的h 坐标从小往大处理处理, 也就是如果有子行, 一定会包含进去的.
                ...
                for k1,v1 in sorted(rowMap.items(), key=lambda d:d[1]['h']):
                    ...
                    if ( ( y > y1 or abs(y-y1) < 1 )  and ( (y+h)<(y1+h1) or abs(y1+h1-y-h) < 1 ) ): # abs 的判断是因为子行和母行上面边或下面边重叠的时候, 坐标有可能不是完全相同, 有可能有微小误差
                        ...
                        # 2.1 子行的cells中添加母行的cells, 这样后面画表的时候就可以进行处理了.
                        c1 = rowIndexMap[k]["cells"]
                        c2 = rowIndexMap[k1]["cells"]
                        rowIndexMap[k]["cells"] = sorted(set( c1 + c2))  # 注意,合并的时候, 第一个元素不需要, 以为那个是最大文字行数
                        rowIndexMap[k]["cells"].insert( 0, c1[0])    # 把当前行的最大文字行数补充道第一个元素位置
                ...
                if ( len(sons) != 0 ):
                    maxlines = 0
                    for rowid in sons:
                        maxlines += rowIndexMap[rowid]['cells'][0]
                    if ( maxlines > rowIndexMap[k]['cells'][0] ):
                        rowIndexMap[k]['cells'][0] = maxlines
            ...
            while( key in keylist ):
                ...
                if ( cellid != -1  ):                                       # 碰到cell了
                    ...
                    if ( not flag ):    #不是在处理表格, 那么下面就是表格, 是表格的第一个cell
                        ...
                        tableMap[tbkey] = {"cells":[lasttxtid,cellid],"rows":[row], "cols":[col]}    # cells 第一个元素用来记录最后一个文本      
    ...
    def buildPageTxt( self, textMap, cellMap, cellIndexMap, tableMap, colIndexMap, rowIndexMap ):
        ...
        try:
            ...
            traceback.print_exc()
            return retbuf
    def buildPageTxt2( self, textMap, cellMap, cellIndexMap, tableMap, colIndexMap, rowIndexMap ):
        retbuf = ""
        try:
            if ( len( textMap ) == 0 ):   # 如果textMap 是空的, 可能前面的处理有问题, 没有获得文本
                return
    ...
    def getRowSplitPos( self, rowIndexMap, colIndexMap, cellIndexMap, rowID ):
        ...
        try:
            ...
            for i in range( 0, cols ):
                cellId = rowIndexMap[rowID][i+1]          # rowIndexMap[rowID]的第0个元素是最大行数
                colId = cellIndexMap[cellId]['col']
                col_len = colIndexMap[colId][0]            # 当前列的最大宽度
                if ( i == 0 ):
                    col_pos = col_len
                else:
                    col_pos += col_len  + 1           # 加 2 是为了 修正分隔符所占的2位, 因为每一个都进行修正, 这样只需要加一次即可, 保证本次是真实的位置即可.以后的也只需要加一次
    ...
    def buildRowSplit( self, rowIndexMap, colIndexMap, rowID, cellMap, cellIndexMap ):
        ...
        try:
            ...
            for i in range(0, len_all ):
                ...
                if ( i == 1 ):      # 第一列不需要修正分隔符的占位
                    con_len = allSplitPos[i] - allSplitPos[i-1]    
                else:
                    con_len = allSplitPos[i] - allSplitPos[i-1] - 1    # 计算连线距离, 减去2是为了修正连线不包括分隔符的占位2, 因为前面的位置也修正过了, 所以这儿只需要减2即可
        ...
        try:
            ...
            for i in range(0, len_cur ):
                ...
                if ( i == 1 ):
                    ...
                    con_len = curRowSplitPos[i] - curRowSplitPos[i-1] - 1    # 计算连线距离
