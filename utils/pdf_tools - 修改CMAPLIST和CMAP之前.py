...
<</Count 266/Kids[3 0 R。。。298 0 R 299 0 R]/Type/Pages>>
    ...
    def decode( self, buf, cmap, desfile ) :
        try :
            ...
            retbuf = ""
            cur_xy, cellMap, cellIndexMap, textMap, tm = {}, {-1:{}}, {}, {}, {}  # 坐标信息, 表格信息(初始化页眉是没有的, -1用于页眉编号), 文本信息, 转换坐标
            ...
            while ( True ):
                ...
                if ( "BT" in tmpbuf ):     # 如果是 包含"BT"/"ET"
                    retbuf += self.processBT( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm )
                    #retbuf += self.processBT( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, stateMap, cur_state )
                    
                if ( "BDC" in tmpbuf ) :     # 如果是 包含"BDC"/"EMC"   
                    retbuf += self.processBDC( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm )
    ...
    def processBT( self, buf, streambuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm ):
        try :
            ...
            while ( True ):
                ...
                if ( ( "/Top" in tmpbuf ) and ( "/Pagination" in tmpbuf )  ) : 
                    ...
                    retbuf += self.processBDC( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm ) # 调用 BT 的处理
                ...
                if ( self.hasText( tmpbuf ) ) :
                    retbuf += self.processText( tmpbuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm )
     ...
     */
    '''
    def processBDC( self, buf, streambuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm ):
        try:
            retbuf = ""
            tmpbuf = buf
            while ( True ) :       
                ...
                if ( ( "/Top" in tmpbuf ) and ( "/Pagination" in tmpbuf )  ): 
                    ...
                    retbuf += self.processBT( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm ) # 调用 BT 的处理
    ...
    # cellMap = {编号:{'x':,'y':,'w':,'h':}}
    # tm = {'a':a, 'b':b, 'c':c, 'd':d, 'e':e, 'f':f}    # 一直存放的是最新的tm数据
    def processText( self, buf, cmap, cur_xy, textMap, cellMap, cellIndexMap, tm ) :
        retbuf = ""
        ...
        if ( len(cellMap) > 1 ):   # >1 是为了跳过页眉
            ...
            if ( ox >= cx and ox <= cx+cw and oy > cy and oy <= cy+ch ):
                ...
                if ( len(retbuf) > maxlen ):    # 给cell的maxlen赋值
...
<0E2A>Tj
        ...
        try :
            ...
            for i in range( 0, len(tmpbuf) ): 
                if ( "re" in tmpbuf[i] ) :              # 如果找到re, 那么前面i-4, i-3, i-2, i-1就是x,y,w,h
                    ...
                    # 2016.12.23: 增加无效cell过滤得处理, 即当前cell在前一个cell内, 则丢弃
                    if ( self.validCell( cellMap, cur_cell ) ):
                        value = cur_cell.copy()   # 2016.12.21, 修改'maxlen'存放到cellIndexMap 之中, 那么processRE()需要增加参数cellIndexMap
                        if ( cur_cell not in cellMap.values() ):
                            cellMap[key] = value
                            cellIndexMap[key] = {"maxlen":-1,"row":-1,"col":-1,"table":-1, "txtlist":[] }    # 缺省长度为-1, 没有对应的文本, 后续处理会增加文本编号

                            with open("F:/F_T_tmp/tmp1/re.txt","a+") as fs:
                                fs.write( "%-2d:buf=%-30s,re: x=%-8.2f,y=%-8.2f,w=%-8.2f,h=%-8.2f\r\n\r\n" % ( key,buf,cur_cell["x"],cur_cell["y"],cur_cell["w"],cur_cell["h"] ))
     ...
     */
    '''
    def validCell( self, cellMap, cur_cell ):
        retflag = True         # 缺省为有效的cell

        try:
            length = len(cellMap)
            if ( length > 1 ):    # 第一个是页眉, 有效的， 页眉的key 值是-1. 所以只需要判断大于1的情况即可, 第一个有效的cell的key值就是1, 同理后面的也一样
                pre_cell = cellMap[length -1].copy()   # 获取上一个有效cell内容
                cx, cy, cw, ch = cur_cell["x"], cur_cell["y"], cur_cell["w"], cur_cell["h"]
                px, py, pw, ph = pre_cell["x"], pre_cell["y"], pre_cell["w"], pre_cell["h"]
                if ( ( cx>= ph and cx+cw <= px+pw ) and ( cy >= py and cy+ch <= py+ph ) ):
                    # x,y 范围都在pre_cell之内的话, 就判定为无效
                    return False
            
                
        except:
            print("validCell() Exception Error!")
            traceback.print_exc()
        return retflag
                ...
                if ( not row  in rowMap.values() ):  # 如果该cell的x,w没有在rowMap 中, 表示是新row
                    ...
                    for k1,v1 in sorted( rowMap.items(), key=lambda d:d[0]) :
                        if ( row == v1 ):  # 找到rowMap 对应的row
                            ...
                            if ( rowIndexMap[k1][0] < len( cellIndexMap[k]["txtlist"] ) ):   # 更新每行单元格的最大行数
                                rowIndexMap[k1][0] = len( cellIndexMap[k]["txtlist"] )
                ...
                if ( cellid != -1  ):                                       # 碰到cell了
                    ...
                    if ( not flag ):    #不是在处理表格, 那么下面就是表格, 是表格的第一个cell
                        ...
                        if ( not cellid in tableMap[tbkey]['cells'] ):
                            ...
                            cellIndexMap[cellid]['table'] = tbkey
                            tableMap[tbkey]["cells"][0] = cellIndexMap[cellid]["txtlist"][-1] # 该表中cell的列表的第一个元素用来记录最后一个txt编号
    ...
    def buildPageTxt( self, textMap, cellMap, cellIndexMap, tableMap, colIndexMap, rowIndexMap ):
        ...
        try:
            ...
            while( i != -1 ):         # 循环里的i 用来记录 next textid
...
└──────────┴────┴────┴────┴────┴────┘
    ...
    def getCellLineText( self, rowId, colNo, lineNo, rowIndexMap, colIndexMap, cellMap, cellIndexMap, textMap ):
        retbuf = ""
        try:
            cellId = rowIndexMap[rowId][ colNo + 1 ]   # 加1是因为第0个元素是该Row 的最大文本行数. 注意, 不同于cell的文本行数
            colId = cellIndexMap[cellId]['col']
            col_len = colIndexMap[colId][0]            # 当前列的最大宽度
            cell_lines = len(cellIndexMap[cellId]["txtlist"])     # 所有的元素个数就是该cell的文本行数, 每一个文本就是一行
            if ( lineNo < cell_lines ) :                     # 如果该cell的行数小于最大行数, 不够的就用空格替代， lineNo 是从0开始的
                txtId = cellIndexMap[ cellId ]["txtlist"][lineNo]
