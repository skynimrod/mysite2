...
class pdfService():
    ...
    def parsePDF( self, srcFile, pageno ):
        ...
        for k,v in sorted( xref.items(), key=lambda d:d[0]):
        ...
        with open("F:/F_t_tmp/tmp1/Rowpages.txt","w") as fs:
            for k,v in sorted( pages.items(), key=lambda d:d[0]): 
                buf =  self.getObjContent( v, xref )
                fs.write( v + "|" + buf + "\r\n\r\n" ) 
        ...
        with open("F:/F_t_tmp/tmp1/xrefAll.txt","w") as fs:
            ...
            for key in sorted(xref.keys()):
                if ( key != "retcode"):
                    ...
                    #print(key)
                    #print(type(key))
                    fs.write( key + "(%d[%08x])||" % (xref[key],xref[key]) + buf + "\r\n\r\n" ) 
        '''
...
<</Count 266/Kids[3 0 R。。。298 0 R 299 0 R]/Type/Pages>>
    ...
    def getObjContent( self, obj, xref ):
        ...
        try:
            if ( len(xref) == 0 ):
                return content
            #print("getObjContent(). xref, obj="+obj)
            #if ( obj == '30'):
            #    print("30对象的位置%d" % xref[obj])
    ...
    def getCmapList( self, pages, xref, cmaplist, contentMap):
        try:
            ...
            for key,v in sorted( pages.items(), key=lambda d:d[0]):
                buf = self.getObjContent( v, xref ).strip()
    ...
    def procType0Stream( self, obj, xref ) :
        ...
        try :
            ...
            content = self.getObjContent( obj, xref ) 
    ...
    def decode( self, buf, cmap, desfile ) :
        try :
            ...
            retbuf = ""
            cur_xy, cellMap, textMap, tm = {}, {-1:{}}, {}, {}  # 坐标信息, 表格信息(初始化页眉是没有的, -1用于页眉编号), 文本信息, 转换坐标
            ...
            while ( True ):
                ...
                if ( "BT" in tmpbuf ):     # 如果是 包含"BT"/"ET"
                    retbuf += self.processBT( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, tm )
                ...
                if ( "re" in tmpbuf ) :     # 如果是 包含"BDC"/"EMC"   
                    self.processRE( tmpbuf, cellMap, cur_xy, tm )
            ...
            retbuf = self.processTxtForTable( textMap, cellMap )  # 处理文本内容, 如果有表格, 则进行表格处理
    ...
    def processBT( self, buf, streambuf, cmap, cur_xy, textMap, cellMap, tm ):
        try :
            ...
            while ( True ):
                ...
                if ( ( "/Top" in tmpbuf ) and ( "/Pagination" in tmpbuf )  ) : 
                    ...
                    retbuf += self.processBDC( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, tm ) # 调用 BT 的处理
                ...
                if ( self.hasText( tmpbuf ) ) :
                    retbuf += self.processText( tmpbuf, cmap, cur_xy, textMap, cellMap, tm )
     ...
     */
    '''
    def processBDC( self, buf, streambuf, cmap, cur_xy, textMap, cellMap, tm ):
        try:
            retbuf = ""
            tmpbuf = buf
            while ( True ) :       
                ...
                if ( ( "/Top" in tmpbuf ) and ( "/Pagination" in tmpbuf )  ): 
                    ...
                    retbuf += self.processBT( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, tm ) # 调用 BT 的处理
    ...
    # cellMap = {编号:{'x':,'y':,'w':,'h':}}
    # tm = {'a':a, 'b':b, 'c':c, 'd':d, 'e':e, 'f':f}    # 一直存放的是最新的tm数据
    def processText( self, buf, cmap, cur_xy, textMap, cellMap, tm ) :
        retbuf = ""
        ...
        # 3. 获取位置信息
        '''
        if ( not "TD" in buf and not "Td" in buf ):  # 没有位置信息, 使用(0,0), 然后通过转换坐标来计算实际坐标
            cur_xy={ "x":0, "y":0, "ox":tm['e'], "oy":tm['f'], "OPR":"TD", "tm":tm.copy() }
        elif ( "TD" in buf ):
        ...
        if ( len(cellMap) > 1 ):   # >1 是为了跳过页眉
            ...
            cell = cellMap[len(cellMap)-1]
            cx,cy,cw,ch = cell['x'], cell['y'], cell['w'], cell['h']
            if ( ox >= cx and ox <= cx+cw and oy > cy and oy <= cy+ch ):
                value['cell'] = len(cellMap)-1   # -1 是因为cellMap 第一个是页眉的cell占用了,len()=2的时候才是编号为1的cell
                if ( len(retbuf) > cell['maxlen'] ):    # 给cell的maxlen赋值
                    #cell['maxlen'] = len(retbuf)   # len() 把中文也当作1个字符长度
                    cell['maxlen'] = self.rlen(retbuf)# 中文算2个字符长度的计算方法
...
<0E2A>Tj
     ...
     */
    '''
    def processRE( self, buf, cellMap, cur_xy, tm ) :
        try :
            ...
            for i in range( 0, len(tmpbuf) ): 
                if ( "re" in tmpbuf[i] ) :              # 如果找到re, 那么前面i-4, i-3, i-2, i-1就是x,y,w,h
                    ...
                    #cur_xy["OPR"] = "re"
                    '''
                    if ( len(tm) != 0 ):   # 如果tm有效, 则需要进行坐标转换
                        x = tm['a'] * cur_cell['x'] + tm['c']*cur_cell['y'] + tm['e']
                        y = tm['b'] * cur_cell['x'] + tm['d']*cur_cell['y'] + tm['f']
                        cur_cell['x'] = round(x,3)
                        cur_cell['y'] = round(y,3)
                    '''

                    # x 坐标小于0可能是控制用的, h,w数值小于1可能是用来做分割线的, (x,y)=(0,0)从原点来时的表格也是控制用(页眉都不可能在原点)
                    # 把页眉也保留, 用来判断页眉文本
                    #if ( cur_cell["x"] < 0 or cur_cell["h"] < 1 or cur_cell["w"] < 1 or ( cur_cell["x"]==0 and cur_cell["y"]==0) ):
                    if ( cur_cell["x"] < 0 or cur_cell["w"] < 1 or ( cur_cell["x"]==0 and cur_cell["y"]==0) ):   # (0,0)仍然过滤, 暂时没想到怎么处理
                        continue
                    if (  cur_cell["h"] < 1 ):  # 页眉的分割线
                        cellMap[-1] = cur_cell.copy()   # 这个是页眉分割线, 实际上是一个高度极小的表格.
                        return

                    cur_cell['maxlen'] = -1     # 表格的缺省长度是-1, 后面根据文本长度来将最大长度赋值.
                    key = len(cellMap)      # 页眉所在的cell 保留了, 页眉的编号是-1
                    value = { cur_cell['x'], cur_cell['y'], cur_cell['w'], cur_cell['h'] }
                    if ( cur_cell not in cellMap.values() ):
                        cellMap[key] = value

                        with open("F:/F_T_tmp/tmp1/re.txt","a+") as fs:
                            fs.write( "buf=%s,re: x=%.2f,y=%.2f,w=%.2f,h=%.2f\r\n" % ( buf,cur_cell["x"],cur_cell["y"],cur_cell["w"],cur_cell["h"] ))


                    cur_xy.clear()
                    tm.clear()
                    # 如果字典本身有数据, 通过简单的赋值{} 是不能清空的.
                    print("======re====cur_xy, tm==??")
                    print(cur_xy)
                    print(tm)
                    return
    ...
    def processTd( self, buf, cur_xy, tm ) :
        try :
            tmpbuf = buf.split(" ")
            if ( "94.76 370.7 Td" == buf ):
                print("processTd() buf=%s , tm=??" % buf)
                print(tm)
                print(cur_xy)
            for i in range( 0, len(tmpbuf) ): 
    ...
    #       buildTableMap()
    #       buildPageTxt()
    def processTxtForTable( self, textMap, cellMap ):
        try:
            retbuf = ""
            ...
            print(retbuf)
            '''
            sbuf, oy, cell, tableflag = "", -1, {}, False   # tableflag 是个表格判断, 如果出现文本在cell， 则为true, 开始处理表格, 如果文本不在
            for k,v in sorted( textMap.items(), key=lambda d:d[0]) :
                if ( v['o_xy']['oy'] < cellMap[-1]['y'] ):   # 过滤页眉
                    print("-----------过滤页眉---------")
                    continue
                if ( k == 1 ):          # 第一个
                    sbuf += v['txt']
                    oy = v['o_xy']['oy']        # oy 记录前一次的oy, 用来进行后续比较判断
                    cellid = v['cell']
                    continue
                if ( k > 1):
                    if ( abs(v['o_xy']['oy']- oy) < 1 ):    # 两个Y坐标小于1视为同一行
                        if ( v['cell'] == -1 ): # 不在表格中, 不用添加空格
                            sbuf += v['txt']
                        else:
                            sbuf += "  |  " + v['txt']
                    else:
                        sbuf += "\r\n"+v['txt']
                        oy = v['o_xy']['oy']    # 换行了, 记录当前oy, 供下次比较判断
                        cellid = v['cell']
    ...
    def delTxt(self, item, textMap, cellIndexMap ):
        ...
        if ( cellId in cellIndexMap.keys() ):
            cellIndexMap[cellId].remove(item)
    ...
    def preProcTxtMap( self, textMap, cellIndexMap ):
        try:
            i, keys = 1, textMap.keys()

            if ( len(textMap) == 0 ):
                return
            
            # a. 找到第一个文本编号, 这个是为了容错, 一般第一个文本的编号就是1
            while( not i in keys ):
                print(keys)
                print(i)
                i += 1
            ...
            # b. 顺序处理不在单元格里面的文本信息
            while( i in keys ):
                if ( textMap[i]['cell'] != -1 ):  # 跳过单元格里面的文本(因为单元格的处理比较复杂, 还有很多中间过程)
                    i += 1
                    continue
                    
                ox, oy = textMap[i]['o_xy']['ox'], textMap[i]['o_xy']['oy']
                pre_id = textMap[i]['pre']
                ...
                if ( pre_id != -1 ):          # 如果前面有文本编号
                    ...
                    if ( abs(pre_oy - oy) < 1 ):    # 两个Y坐标小于1视为同一行
                        if ( pre_ox <= ox ):                  # 留小不留大, 小的在前面
                            textMap[pre_id]['txt'] += textMap[i]['txt']
                            self.delTxt( i, textMap, cellIndexMap )
                        else:
                            textMap[i]['txt'] += textMap[pre_id]['txt']
    ...
    def preProcTxtMap4Cell( self, textMap, cellMap, cellIndexMap ):
        try:
            for k,v in sorted( cellMap.items(), key=lambda d:d[0]):
                if (len( cellIndexMap[k] ) <= 1 ):  # cell 中多余1个文本的才进行处理, 单个文本不存在是否需要拼接的问题
                    continue
                
                txtlist = cellIndexMap[k].copy()
                ...
                for item in sorted( txtlist ):
                    ...
                    if ( ox >= preTxt["ox"] ):              # 留小删大
                        ...
                        if ( cellMap[k]["maxlen"] < length ):
                            cellMap[k]["maxlen"] = length
                ...
                if ( not col  in colMap.values() ):  # 如果该cell的x,w没有在colMap 中, 表示是新col
                    ...
                    for k1,v1 in sorted( colMap.items(), key=lambda d:d[0]) :
                        if ( col == v1 ):  # 找到colMap 对应的col
                            ...
                            if ( colIndexMap[k1][0] < cellMap[k]["maxlen"] ):   # 修正col 的最大长度
                                colIndexMap[k1][0] = cellMap[k]["maxlen"]
                            cellMap[k]['col'] = k1

                key = len(rowMap)+1
                if ( not row  in rowMap.values() ):  # 如果该cell的x,w没有在rowMap 中, 表示是新row
                    rowMap[key] = row
                    maxlines = len( cellIndexMap[k] )
                    rowIndexMap[key] = [ maxlines, k]  # rowIndexMap 的第一个元素是当前行的最大文本行数
                    cellMap[k]['row'] = key
                else:
                    for k1,v1 in sorted( rowMap.items(), key=lambda d:d[0]) :
                        if ( row == v1 ):  # 找到rowMap 对应的row
                            rowIndexMap[k1].append(k)       # 如果对应的row 已经存在, 则在rowIndexMap中添加该cell 编号即可
                            if ( rowIndexMap[k1][0] < len( cellIndexMap[k] ) ):   # 更新每行单元格的最大行数
                                rowIndexMap[k1][0] = len( cellIndexMap[k] )
                            cellMap[k]['row'] = k1
    ...
    def buildTableMap( self, textMap, cellMap, cellIndexMap, tableMap, colIndexMap, rowIndexMap  ):
        try:
            ...
            keylist = textMap.keys()

            if ( len(keylist) == 0 ):  # 没有内容就返回, 这儿是为了容错
                return
            
            #while( (not key in keylist) or (textMap[key]["pre"] != -1 ) ):  # 循环找第一个文本编号
            while( key not in keylist or textMap[key]["pre"] != -1 ):  # 循环找第一个文本编号
                key += 1
                
            while( key in keylist ):
                cellid = textMap[key]['cell']
                if ( cellid != -1  ):                                       # 碰到cell了
                    row, col, tbkey = cellMap[cellid]["row"], cellMap[cellid]["col"], -1
                    if ( not flag ):    #不是在处理表格, 那么下面就是表格, 是表格的第一个cell
                        tbkey = len(tableMap)+1
                        # 第一个用来记录本表最后用到的txt编号, 用来显示文本的时候进行处理
                        tableMap[tbkey] = {"cells":[0,cellid],"rows":[row], "cols":[col]}          
                        cellMap[cellid]['table'] = tbkey
                        flag = True
                    else:         # 已经在处理表格了, 那么这个cell仍然是table 的一部分
                        tbkey=len(tableMap)     # 不是新建的,不用+1
                        if ( not cellid in tableMap[tbkey]['cells'] ):
                            tableMap[tbkey]["cells"].append( cellid )
                            cellMap[cellid]['table'] = tbkey
                            tableMap[tbkey]["cells"][0] = cellIndexMap[cellid][-1] # 该表中cell的列表的第一个元素用来记录最后一个txt编号
    ...
    def buildPageTxt( self, textMap, cellMap, cellIndexMap, tableMap, colIndexMap, rowIndexMap ):
        ...
        try:
            ...
            i = 1
            # 找到第一个有效的文本编号,一般就是1, 这儿的处理是为了容错
            while( i not in textMap.keys() or textMap[i]['pre'] != -1 ):
                i += 1
            
            while( i != -1 ):
                cellId = textMap[i]['cell']
                if ( cellId == -1 ):   # 不在cell中的文本本身就是单独一行
                    retbuf += textMap[i]['txt'] + '\r\n'
                    i = textMap[i]['next']
                else:  # 表格处理
                    tableid = cellMap[cellId]['table']
                    retbuf += self.buildTableTxt( textMap, tableMap[tableid], cellMap, cellIndexMap, colIndexMap, rowIndexMap )
                    txtid = tableMap[tableid]['cells'][0]    # 记录该表的最后一个文本, 用来查找下一个文本编号
                    i = textMap[txtid]['next']
            return retbuf
        except:
    ...
    def buildTableTxt( self, textMap, table, cellMap, cellIndexMap, colIndexMap, rowIndexMap ):
        ...
        try:
            ...
            for item in rows:      # item 是行号
                if ( rows.index(item) == 0 ):  # 第一个需要建表头
           ...
           #retbuf += self.buildRowFooter( rowIndexMap, colIndexMap, rows[-1], textMap, cellMap, cellIndexMap )
        except:
    ...
    def buildRowHeader( self, rowIndexMap, colIndexMap, rowID, textMap, cellMap, cellIndexMap ):
        ...
        try:
            ...
            tableDict = {'left_top':'┌', 'mid_top':'┬', 'right_top':'┐', 'mid_left':'├', 'mid_mid':'┼',
                         ...
                         'rowline':'─', 'colline':'│'}
            # 1. 先取该行的列数与单元编号, rowIndexMap[rowId]是个列表   [列数,cellid1,cellid2...]
            cols = len( rowIndexMap[rowID] ) -1 #  几个单元格就是几列, -1 是因为第一个元素是行数
            lines = rowIndexMap[rowID][0]
            
            print("行号:%d, 列数:%d, 行数:%d " % ( rowID, cols, lines ) )
            print(rowIndexMap[rowID])

            retbuf = tableDict['left_top']
            for i in range(0,cols):
                cellId = rowIndexMap[rowID][i+1]
                colId = cellMap[cellId]['col']
                col_len = colIndexMap[colId][0] 
                if ( cellId != rowIndexMap[rowID][-1] ):  # 如果不是最后一个cellID, 就用rowline+midt_op
    ...
    def buildRowBody( self, rowIndexMap, colIndexMap, rowID, textMap, cellMap, cellIndexMap ):
        ...
        try:
            '''
           │主要指标            │16-09-30│16-06-30│16-03-31│15-12-31│15-09-30│
           ├──────────┼────┼────┼────┼────┼────┤ 
｜基本每股收益(元)    ｜  0.2500｜  0.2577｜  0.0800｜  0.3918｜  0.5300｜
｜基本每股收益扣除(元)｜       -｜  0.2447｜       -｜  0.3843｜       -｜
｜每股净资产(元)      ｜  3.6292｜  5.4207｜  5.2223｜  3.0332｜  7.0350｜
｜加权净资产收益率%   ｜  8.0800｜  5.9600｜  2.5100｜ 18.9000｜ 11.4000｜
｜营业收入（亿元）    ｜   21.35｜   13.69｜    3.88｜   22.11｜   12.70｜
｜营业收入增长率%     ｜   68.09｜  114.33｜   21.94｜   79.17｜   70.48｜
｜净利润(亿元)        ｜    6.13｜    4.03｜    1.09｜    5.33｜    2.60｜
｜净利润增长率%       ｜  136.36｜  273.04｜   99.09｜  102.76｜  206.72｜
｜报告期股本(亿股)    ｜   25.62｜   17.07｜   17.07｜   14.17｜    5.66｜
└──────────┴────┴────┴────┴────┴────┘
            '''
            # ┌──────────┬────┬────┬────┬────┬────┐
            tableDict = {'left_top':'┌', 'mid_top':'┬', 'right_top':'┐', 'mid_left':'├', 'mid_mid':'┼',
    ...
    def getRowSplitPos( self, rowIndexMap, colIndexMap, cellMap, rowID ):
        retlist = [0]
        try:
            cols = len( rowIndexMap[rowID] ) -1
            col_pos = 0
            for i in range( 0, cols ):
                cellId = rowIndexMap[rowID][i+1]
                colId = cellMap[cellId]['col']
    ...
    def getCellLineText( self, rowId, colNo, lineNo, rowIndexMap, colIndexMap, cellMap, cellIndexMap, textMap ):
        retbuf = ""
        try:
            cellId = rowIndexMap[rowId][ colNo + 1 ]
            colId = cellMap[cellId]['col']
            col_len = colIndexMap[colId][0]            # 当前列的最大宽度
            cell_lines = len(cellIndexMap[cellId])     # 所有的元素个数就是该cell的文本行数, 每一个文本就是一行
            if ( lineNo < cell_lines ) :                     # 如果该cell的行数小于最大行数, 不够的就用空格替代
                txtId = cellIndexMap[ cellId ][lineNo]
    ...
    def buildRowSplit( self, rowIndexMap, colIndexMap, rowID, cellMap, cellIndexMap ):
        ...
        try:
            ...
            preRowID = rowID -1 
            preRowSplitPos = self.getRowSplitPos( rowIndexMap, colIndexMap, cellMap, preRowID )
            
            curRowSplitPos = self.getRowSplitPos( rowIndexMap, colIndexMap, cellMap, rowID )
            ...
            j, k = 0, 0         # 计数器, j对应上一行, k对应当前行. 对于只有其中一行有的情况, 要在另一行删除等量 '┴'或'┬'的连接线'-'长度
            for i in range(0, len_all ):
                if ( i == 0 ):      # i == 0的时候就是位置0, 用来方便后续计算控制用的, 没有实际意义
                    continue

                curPos = allSplitPos[i]
                con_len = allSplitPos[i] - allSplitPos[i-1]     # 计算连线距离

                #print("curpos=%d, con_len=%d,:%s:" % (curPos, con_len, retbuf) )
                if ( curPos == preRowSplitPos[-1]):
                    flag1 = True
                if ( curPos == curRowSplitPos[-1]):
                    flag2 = True
                    
                if (  curPos in preRowSplitPos and curPos in curRowSplitPos ): # 两行都有
                    retbuf += (con_len-j-k) * tableDict['rowline'] 
                    ...
                    if ( curPos in preRowSplitPos ) :    # 在上一行
                        j += 1
                        retbuf += (con_len-k) * tableDict['rowline'] 
                        if ( not flag1 ):  # 上一行没结束, 无论下一行是否结束, 都是'------┴'
                            retbuf += tableDict['mid_bottom']
                        else:  # 2行都结束了, 但是最后是上一行, 因为外围的判断是上一行, 用'------┘'
                            retbuf += tableDict['right_bottom']
                    else:          # 不在上一行就是当前行
                        k += 1
                        retbuf += (con_len-j) * tableDict['rowline'] 
    ...
    #      将表格里面的文本格式化输出
    def buildRowFooter( self, rowIndexMap, colIndexMap, item, textMap, cellMap, cellIndexMap ):
        retbuf = ""
        try:
            # ┌──────────┬────┬────┬────┬────┬────┐
            tableDict = {'left_top':'┌', 'mid_top':'┬', 'right_top':'┐', 'mid_left':'├', 'mid_mid':'┼',
            ...
            for i in range(0, lines):    #外循环是行
                retbuf += tableDict['body_left']
                for j in range(0,cols):         # 内循环是列,
                    cellId = rowIndexMap[rowID][j+1]
                    colId = cellMap[cellId]['col']
                    col_len = colIndexMap[colId][0]            # 当前列的最大宽度
                    cell_lines = len(cellIndexMap[cellId])     # 所有的元素个数就是该cell的文本行数, 每一个文本就是一行
                    print(" 行:%d,列:%d, cell_lines=%d, cellId=%d, col_len=%d, i=%d, j=%d " % \
                          (rowID, colId, cell_lines, cellId, col_len, i, j ) )
                    print(cellIndexMap[cellId])
                    if ( i < cell_lines ) :                     # 如果该cell的行数小于最大行数, 不够的就用空格替代
                        txtId = cellIndexMap[ cellId ][i]
                        txt = textMap[txtId]['txt']
                        blank_len = col_len - self.rlen(txt)
                        retbuf += txt + blank_len * " "
                    else:
...
pdf.parsePDF('F:/F_T_tmp/1218.pdf', '1')
