...
from sheet_tools import sheetService
...
class pdfService():
    ...
    def __init__(self):
        ...
        self.cmaps = {}
        self.contentMap = {}
        self.tableContentMap = {}  
        
    ####################################################
    # W 开头的方法属于外部接口
    # W1. getSpecKeyLine()
    #     获取指定关键字所在的行, 只返回第一个包含指定关键字的行
    def getSpecKeyLine( self, key ):
        pagesum = len(self.pages)
        print("页数:%d" % pagesum)
        retbuf = ""
        try:
            for i in range(1,pagesum+1):
                retbuf = self.processPage( str(i), "F:/F_t_tmp/tmp1/"+ str(i) + "_content.txt")
                if ( key in retbuf ):
                    tmpbuf = retbuf.split("\r\n")

                    for j in range(0,len(tmpbuf) ) :
                        if ( key in tmpbuf[j] ):
                            print("======================找到%s所在的行===========(第%d页)" % (key, i) )
                            print( tmpbuf[j] )
                            return tmpbuf[j]
                    
                    return retbuf
        except:
            print("getSpecKeyLine() Exception Error!")
            traceback.print_exc()
        return retbuf

    # 获取页数
    def getPagesSum( self ):
        return len(self.pages)

    def getSpecPage( self, pageno ):
        #pageno = "3"
        with open(self.tmpfile1, "w") as fs:
            fs.write("------第%s页测试--------------\r\n" % pageno )
            
        retbuf = self.processPage( pageno, "F:/F_t_tmp/tmp1/"+pageno + "_content.txt")
        print("##---------------##")
        print(retbuf)
        return retbuf

    # A1. 应用接口1, 实际发布的时候, 这个接口是不封装在该中间件里面的, 而是实际使用该中间件的例子
    # getNDYJKB()
    #     获取年度业绩快报   YeJiKuaiBao 的汉语拼音首字母     这类公告提供的是未经过会计师事务所核准的报告,  可以参考
    # 说明:
    #     1. 业绩快报公告名称的检索关键字: 年份, 例如:"2017",  特征,例如:"年度业绩快报", "年度业绩预告"
    #        mysql> select * from report_reportlist_2017_tbl where reportname like "%年度业绩快报%" order by releaseddate asc limit 10;
    #     2. 公告内容检索关键字: ""
    #       "年度业绩预告" 公告内容的关键字为:  "比上年同期增长约" 后面是利润变化百分比, 有可能是负数
    #       "年度业绩快报" 公告内容则要丰富的多, 包含一些财务数据.  "归属于上市公司股  东的净利润" 是分成2行来存放的.
    #     3. 存放年度业绩快报的表   ndyjkb_2017_tbl
    def getNDYJKB( self, year ):
        
        return

    # A2. 年度业绩预告
    #   
    #   1. 公告名称检索关键字:   年份: "2017年",  "年度业绩预告"
    #    "年度业绩预告" 公告内容如下:
    #   3、业绩预告情况表 
    #
    #┌───────────────┬────────────────────────────────────┬────────────────────┐
    ...
    #│               ├────────────────────────────────────┤                    │
    #│归属于上市公司  │盈利：116,388.73万元-133,015.70万元 |盈利：83,134.81万元 │
    #└───────────────┴────────────────────────────────────┴────────────────────┘
    #  说明:
    #     年度业绩预告 公告, 属于公司提前的业绩预告, 提前于"年度业绩快报".
    #     这类公告格式并不是完全一致, 比如: 870104(飞拓无限) 与 300017 的年度预计预告格式就有区别
    #  处理方式:
    #     0. 获取 "比上年同期增长：" 所在行的内容
    #     1. 获取 "比上年同期增长："后面的百分比内容
    #     2. 获取 "盈利："后面的金额内容, 并对总股数相除, 获取每股收益.
    def getNDYJYG( self ):
        key = "比上年同期增长："
        buf = self.getSpecKeyLine( key )
        tmpbuf = buf.split( key )    # 利用key 来拆分字符串, 后面的部分开始就是百分比内容
        tmpbuf2 = tmpbuf[1].split("%")   # 处理 "40%-60%             |                    │"
        print("-----getNDYJYG()----------------")
        print(tmpbuf2)
        tmpbuf2[1] = tmpbuf2[1][1:len(tmpbuf2[1])]    # -60% 中的"-" 是连接符, 过滤掉
        print(tmpbuf2)

        print('显示下表格')
        print(self.sheets)
        print(self.sheets.getSheets())
        return
    ...
    # A4. 获取新股信息
    # mysql> select * from report_reportlist_2017_tbl where releaseddate = '2017-02-13' and stockcode = '-';
    # mysql> select * from report_reportlist_2017_tbl where releaseddate = '2017-02-13' and reportname like '%首次公开发行股票招股意向书摘要%';
    #   "首次公开发行股票招股意向书摘要"
    # 处理的主要是"第二节 本次发行概况" 部分. "注册资本"(即总股本)  "发行股数" 所在行, "发行前每股净资产", "发行后每股净资产" "发行市盈率", "市净率"
    #  "常青股份首次公开发行股票发行安排及初步询价公告----2017-02-13 "
    #       "T日", "股票简称", "股票代码" "网上申购代码", "所在行业" (例如:"常青股份所在行业为汽车制造业（C36）"  )
    #  初步询价及网下发行由主承销商通过上交所申购平台组织实施
    #      上交所申购平台网址为：https://120.204.69.22/ipo。请符合资格的网下投资者通过上述网址参与本次发行的初步询价和网下申购。
    #  通过申购平台报价、查询的时间为初步询价和网下申购期间每个交易日9:30-15:00。关于申购平台的相关操作办法请查阅上交所网站（
    #  www.sse.com.cn）—服务—IPO业务专栏中的《上海市场首次公开发行股票网下发行实施细则》、《网下IPO系统用户手册（申购交易员分册）》等相关规定。
    #      "网下初始发行数量"
    #      "网下投资者及其管理的配售对象报价应当包含每股价格和该价格对应的拟申购股数，且只能有一个报价，其中非个人投资者应当以机构为单位进行报价。"
    #      "综合考虑本次初步询价阶段网下初始发行数量及主承销商对发行人的估值情况，主承销商将网下投资者管理的每个配售对象最低拟申购数量设定为500万股，
    #   拟申购数量最小变动单位设定为10万股，即网下投资者管理的每个配售对象的拟申购数量超过500万股的部分必须是10万股的整数倍，且不超过3,060万股。
    #   配售对象报价的最小单位为0.01元。8、主承销商对所有配售对象的报价按照"
    ...
    def parsePDF( self, srcFile ):
        ...
        print(self.pages)
        print( "========= cmaplist size ====%d" % len(self.cmaplist) )
        print( "========= cmaplist ====")
        print( sorted(self.cmaplist.items(),key=lambda d:d[0]) )
        print( "========= contentMap ====%d" % len( self.contentMap) )
        print( sorted( self.contentMap.items(),key=lambda d:d[0]) )
...
<</Count 266/Kids[3 0 R。。。298 0 R 299 0 R]/Type/Pages>>
    ...
    def getCmapList( self ):
        try:
            self.cmaplist["total"] = {}      # cmaplist 哈希表的total 项是所有cmap对象的汇总， 便于后续处理获得CMAP
                        ...
                        if ( "[" in tmp[i] ):        # 如果有中括号, 说明该页对应的内容对象可能是多个
                            ...
                            for j in range(0, len(s) ):
                                if ( i % 3 == 0 ):     # 只取对象编号
                                    tmp1 += s[i] + " "
                            # 删除尾部多余的空格
                            self.contentMap[ self.pages[key] ] = tmp1.strip()
                        else:
                            self.contentMap[ self.pages[key] ] = tmp[i].split(" ")[1]    # 类似 Contents 101 0 R
                    ...
                    if ( "Font" in tmp[i] ) :
                        ...
                        while ( i < len(tmp) ):
                            ...
                            if ( self.isType0( obj ) ) :   # 判断是否是Type0 字体
                                ...
                                if ( obj not in self.cmaplist["total"].keys() ):  # 如果没有在total 项中登记, 则添加 
                                    self.cmaplist["total"][obj] = fontName
                            ...
                            i += 1
                        # 删除尾部多余的空格
                        self.cmaplist[ self.pages[key] ] = cmap.copy()
    ...
    def getCMAPs( self ):
        try:
            # 如果self.cmaplist["total"]里面没有对象编号, 说明没有中文等需要编解码的内容,不需要cmap, 直接返回
            print( 'begin of getCMAPs() ')
            if ( len( self.cmaplist["total"] ) == 0 ) :      
                return 
                
            print( '---------- getCMAPs()  total:---------------' )
            print( self.cmaplist["total"] )
            for obj,fontname in sorted(self.cmaplist["total"].items(), key=lambda d:d[0]) :           # 是个dict, 每一项是 "对象编号":"字体名称", 例如: "8":"C2_0"
                ...
                #print( cmap )
                self.cmaps[fontname] = cmap      # 由原来的对象作为key, 修改为 字体名称作为key, 因为后面的解码的时候是要根据字体名称来解码的
            
            print( "所有的映射内容为：" )
            #print( cmaps )
            print( 'next of getCMAPs() %s' % self.cmaplist["total"] )
        except:
     ...
     */
    '''
    def processPage( self, PageNo, desFile ):
        retbuf = ''
        try :
            desbuf = ''
            
            buf = self.cmaplist[self.pages[PageNo]]      # 该页面用到的CMAP, 有可能为空, 即全英文文档
            #print( "第" +PageNo+"页("+self.pages[PageNo]+")--cmap对象有:" + buf + ":" )
            # 获取内容对象, 有可能是多个
            buf = self.contentMap[ self.pages[PageNo] ]
            ...
            for i in range( 0, len(tmp) ): 
                ...
                retbuf = self.decode( desbuf, desFile )
        ...
        try :
            ...
            while ( True ):
                ...
                if ( "BT" in tmpbuf ):     # 如果是 包含"BT"/"ET"
                    retbuf += self.processBT( tmpbuf, streambuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf )
                    #retbuf += self.processBT( tmpbuf, streambuf, cmap, cur_xy, textMap, cellMap, stateMap, cur_state )
                    
                if ( "BDC" in tmpbuf ) :     # 如果是 包含"BDC"/"EMC"   
                    retbuf += self.processBDC( tmpbuf, streambuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf )
            ...
            #self.sheets.getData( textMap, cellMap, cellIndexMap )
            self.sheets = sheetService( textMap, cellMap, cellIndexMap )
            retbuf = self.sheets.getText()
    ...
    def processBT( self, buf, streambuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf ):
        try :
            ...
            while ( True ):
                ...
                if ( ( "/Top" in tmpbuf ) and ( "/Pagination" in tmpbuf )  ) : 
                    ...
                    print("==BT=========:%s: =======" % tmpbuf )
                    retbuf += self.processBDC( tmpbuf, streambuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf ) # 调用 BT 的处理
                ...
                if ( self.hasText( tmpbuf ) ) :
                    retbuf += self.processText( tmpbuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf )
     ...
     */
    '''
    def processBDC( self, buf, streambuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf ):
        try:
            retbuf = ""
            tmpbuf = buf
            print('-----processBDC():')
            ...
            while ( True ) :       
                ...
                if ( ( "/Top" in tmpbuf ) and ( "/Pagination" in tmpbuf )  ): 
                    ...
                    retbuf += self.processBT( tmpbuf, streambuf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf ) # 调用 BT 的处理
    ...
    # cellMap = {编号:{'x':,'y':,'w':,'h':}}
    # tm = {'a':a, 'b':b, 'c':c, 'd':d, 'e':e, 'f':f}    # 一直存放的是最新的tm数据
    def processText( self, buf, cur_xy, textMap, cellMap, cellIndexMap, tm, tf ) :
        retbuf = ""
        try:
            if ( tf["font"] in self.cmaps.keys() ):
                cmap = self.cmaps[ tf["font"] ].copy()
            else:
                cmap = {}
...
pdf.parsePDF('F:/F_T_tmp/0114.pdf')
pdf.getSpecPage('1')
pdf.getSpecKeyLine( "比上年同期增长" )
