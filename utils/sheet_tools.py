...
class sheetService():
    ...
    def __init__(self, textMap={}, cellMap={}, cellIndexMap={} ):
        ...
        # 1. 容错, 如果 textMap ={} or cellMap == {} or cellIndexMap = {} 那么就不进行下面的处理
        if ( len(textMap) > 0 ):
            print("为空")
            self.text = self.processTxtForTable( self.textMap, self.cellMap, self.cellIndexMap )
        print("------初始化结束--------")
        print(self.sheetMap)
    ...
    def getData(self, textMap, cellMap, cellIndexMap ):
        self.textMap = textMap.copy()
        self.cellMap = cellMap.copy()
        self.cellIndexMap = cellIndexMap.copy()
        # 1. 容错, 如果 textMap ={} or cellMap == {} or cellIndexMap = {} 那么就不进行下面的处理
